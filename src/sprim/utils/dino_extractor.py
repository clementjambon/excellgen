...
# Adapted from "EmerNeRF"
# https://github.com/NVlabs/EmerNeRF/tree/main
...
def extract_and_save_features(
    ...
    extraction_shape = list(extraction_shape)

    if mode == "crop":
        assert (
            input_shape[0] >= extraction_shape[0]
            and input_shape[1] >= extraction_shape[1]
        )
        ...
        offset = (
            input_shape[0] - extraction_shape[0],
            input_shape[1] - extraction_shape[1],
        )
        pad_inf = (offset[0] // 2, offset[1] // 2)
        pad_sup = (offset[0] - pad_inf[0], offset[1] - pad_inf[1])

        def pad(x):
            return F.pad(
                x.permute(0, 3, 1, 2),
                pad=(pad_inf[0], pad_sup[0], pad_inf[1], pad_sup[1]),
                mode="replicate",
            ).permute(0, 2, 3, 1)

        coord_remap = lambda x, y: (
            torch.clip((x - pad_inf[0]) // stride, 0, input_shape[0] // stride - 1),
            torch.clip((y - pad_inf[1]) // stride, 0, input_shape[1] // stride - 1),
        )

    else:
        assert False
        # Check that same aspect ratio
        assert (
            input_shape[0] * extraction_shape[1] == input_shape[1] * extraction_shape[0]
        )
        prep = transforms.Compose(
            [
                transforms.ToTensor(),
                transforms.Resize(extraction_shape),
                transforms.Normalize(
                    mean=[0.485, 0.456, 0.406],
                    std=[0.229, 0.224, 0.225],
                ),
